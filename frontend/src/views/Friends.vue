  ...
  <div>
    ...
    <el-card shadow="never" style="margin-bottom: 16px">
      <div style="display: flex; gap: 10px; align-items: center">
        <el-select
          v-model="addUserId" filterable remote :remote-method="searchAll"
...
async function addFriend() {
  const u = userOptions.value.find(x => x.id === addUserId.value)
  if (!u) return ElMessage.warning('请先搜索并选择用户')
  adding.value = true
  try {
